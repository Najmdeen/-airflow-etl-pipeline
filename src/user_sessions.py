...
with DAG(
    ...
    def get_new_users(**context):
        ...
        response = requests.get(
            f"http://34.233.215.7:8000/users?start_date={start_date}&end_date={end_date}"
        )
        ### END CODE HERE ###
    ...
    def get_session(**context):
        ...
        # APIEndpoint to fill the `<API_ENDPOINT>` placeholder.
        response = requests.get(
            f"http://34.233.215.7:8000/sessions?start_date={start_date}&end_date={end_date}"
        )
        ### END CODE HERE ###
    ...
    def get_session_info_dict(date: str):
        ...
        ### START CODE HERE ### (1 line of code)
        # Use the `"Body"` key in the response dictionary from `session_info_file`.
        # Then, use the `read()` method to obtain
    ...
    def get_user_info(**context):
        ...
        for session in session_info:
            ...
            response = requests.get(
                f"http://34.233.215.7:8000/users_by_id?user_ids={user_id}"
            )
